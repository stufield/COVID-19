...
# COVID data
data <- readr::read_csv("data/covid19_case_summary_2020-04-12.csv")
...
time_data <- data %>% 
  dplyr::filter(description == "Case Counts by Reported Date" &
                  !is.na(value)) %>% 
  dplyr::mutate(
...
dplyr::filter(data, description == "COVID-19 in Colorado by Sex" &
